...
func finish_drag():
	...
	if card_slot_found and not card_slot_found.card_in_slot and !played_card_this_turn:
		...
		$"../BattleManager".player_score += card_being_dragged.points		
		$"../PlayerScore".text = str($"../BattleManager".player_score)
		$"../BattleManager".get_winner()
...
func on_hovered_off_card(card):
	if !card.card_slot_card_is_in:
		if !card_being_dragged:
			...
			var new_card_hovered = raycast_check_for_card_slot()
			if new_card_hovered:
...
func get_card_with_highest_z_index(cards):
	#Assume the first card in cards array has th highest z index
	var highest_z_card = cards[0].collider.get_parent()
	var highest_z_index = highest_z_card.z_index
	
	#Loop throught the rest checking for the highest z index
